...
@click.option('--site', help='site name')
@click.argument('limits', nargs=-1, type=click.Choice(['emails', 'space', 'users', 'email_group', 'expiry', 'support_email', 'support_chat', 'upgrade_url', 'daily_emails', 'cancellation_url', 'companies']))
@pass_context
...
@click.option('--site', help='site name')
@pass_context
def create_demo(context, site):
    from frappe.commands.create_demo import create_demo

    if not site:
        site = get_site(context)

    frappe.connect(site=site)
    print('Creando demo en {}'.format(site))
    create_demo()
