		...
		>
			<div v-if="!is_dragging">
				<div class="text-center">
					{{ __('Arrastrar y soltar los archivos aquí o subir desde') }}
				</div>
				<div class="mt-2 text-center">
					<button class="btn btn-file-upload" @click="browse_files">
						...
						<div class="mt-1">{{ __('Mi dispositivo') }}</div>
					</button>
					<input
						type="file"
						class="hidden"
					...
					<button class="btn btn-file-upload" v-if="!disable_file_browser" @click="show_file_browser = true">
						...
						<div class="mt-1">{{ __('Librería') }}</div>
					</button>
		...
		<div class="file-preview-area" v-show="files.length && !show_file_browser && !show_web_link">
			...
			<div class="flex align-center" v-if="show_upload_button && currently_uploading === -1">
				...
				>
					...
					<span v-else>
						{{ __('Subir {0} archivos', [files.length]) }}
					</span>
				</button>
				<div class="text-muted text-medium">
					{{ __('Pulse el icono del candado para alternar entre público y privado') }}
				</div>
			</div>
		</div>
		<FileBrowser
